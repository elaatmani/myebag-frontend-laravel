<template>
  <div class="tw-pb-3">
    ...
    <!-- Create a size type -->
    <popup :visible="createStatusPopup" @cancel="createStatusPopup = false">
        <CreateOrderStatus @cancel="createStatusPopup = false" />
    </popup>
            ...
            <tbody v-if="items.length > 0">
                <tr v-for="item in items" :key="item.id" :class="[items[items.length - 1].id == item.id && '!tw-border-b-0']" class="tw-bg-white dark:tw-bg-neutral-800 tw-border-b dark:tw-border-b-neutral-700 tw-whitespace-nowrap hover:tw-bg-gray-50 dark:hover:tw-bg-black/30">
                    ...
                    <td class="tw-px-6 tw-py-2 tw-max-w-[120px] tw-truncate">
                        <div v-if="item.mark_as_paid">
                            <v-icon class="tw-text-emerald-400">mdi-check</v-icon>
                        </div>
                        <div v-if="!item.mark_as_paid">
                            <v-icon class="tw-text-red-400">mdi-close</v-icon>
                        </div>
                    </td>
...
<script>
import OrderStatusActions from '@/components/dashboard/settings/order/OrderStatusActions'
import CreateOrderStatus from '@/components/dashboard/settings/order/CreateOrderStatus'

export default {
    // props: ['allItems'],

    components: { OrderStatusActions, CreateOrderStatus },
