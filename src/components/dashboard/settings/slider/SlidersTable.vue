<template>
  <div class="tw-pb-3">
    ...
    <popup :visible="createPopup" @cancel="createPopup = false">
        <CreateSlider @cancel="createPopup = false" />
    </popup>
        ...
        <table class="tw-w-full  tw-relative tw-text-sm tw-text-left !tw-rounded-lg tw-text-gray-500 dark:tw-text-neutral-200">
            ...
            <tbody v-if="items.length > 0">
                <tr v-for="item in items" :key="item.id" :class="[items[items.length - 1].id == item.id && '!tw-border-b-0']" class="tw-bg-white dark:tw-bg-neutral-800 tw-border-b dark:tw-border-b-neutral-700 tw-whitespace-nowrap hover:tw-bg-gray-50 dark:hover:tw-bg-black/30">
                    ...
                    <td>
                        <div class="tw-py-1 tw-px-6 tw-flex tw-items-center tw-gap-2">
                            <img class="tw-w-[150px] tw-rounded" :src="$backend(item.desktop_image_path)" alt="">
                            <img class="tw-w-[80px] tw-rounded" :src="$backend(item.mobile_image_path)" alt="">
                        </div>
                    </td>
                    <td class="tw-px-6 tw-py-1">
                        <p class="tw-max-w-[300px] tw-truncate">{{ item.name }}</p>
                    </td>
                    <td class="tw-px-6 tw-py-1">
                        <p class="tw-max-w-[300px] tw-truncate">{{ item.link }}</p>
                    </td>
                    <td class="tw-py-1 tw-px-6">
                        <SliderActivator :item="item" />
                    </td>
                    <td class="tw-px-6 tw-py-1">
                        <SliderActions :slider="item" />
                    </td>
                </tr>
...
<script>
import CreateSlider from '@/components/dashboard/settings/slider/CreateSlider'
import SliderActivator from '@/components/dashboard/settings/slider/SliderActivator'
import SliderActions from '@/components/dashboard/settings/slider/SliderActions'

export default {
    // props: ['allItems'],

    components: { CreateSlider, SliderActivator, SliderActions },
    ...
    data() {
        return {
            ...
            columns: [ 'id', 'image', 'name', 'link', 'active', 'actions'],
        }
    },
