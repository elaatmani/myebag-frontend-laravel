<template>
  <div class="tw-pb-3">
    ...
    <!-- Create a size type -->
    <popup :visible="createSizePopup" @cancel="createSizePopup = false">
        <CreateSize @cancel="createSizePopup = false" />
    </popup>
                ...
                <tr v-for="item in items" :key="item.id" :class="[items[items.length - 1].id == item.id && '!tw-border-b-0']" class="tw-bg-white dark:tw-bg-neutral-800 tw-border-b dark:tw-border-b-neutral-700 tw-whitespace-nowrap hover:tw-bg-gray-50 dark:hover:tw-bg-black/30">
                    ...
                    <td class="tw-px-6 tw-py-2 tw-max-w-[120px] tw-truncate">
                        <div class="tw-flex tw-items-center tw-flex-wrap tw-gap-2">
                            <div v-for="size in item.sizes" :key="size.id" class="tw-rounded tw-bg-[rgba(var(--primary),.1)] tw-text-[rgb(var(--primary))] dark:tw-text-[rgb(var(--primary))] tw-px-3 tw-border tw-border-solid tw-border-[rgba(var(--primary),.2)] tw-font-medium dark:tw-border-[rgba(var(--primary),.3)] ">
...
export default {
    ...
    components: { SizeActions, CreateSize },
